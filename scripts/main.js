		...
		return {
			...
			link: function bvSheetLink(scope, elem) {
				...
				scope.enterFullscreen = function () {
					if (!document.fullscreenElement && !document.mozFullScreenElement && !document.webkitFullscreenElement && !document.msFullscreenElement && !angular.element(document.querySelector('body')).hasClass('fullscreen-mode')) {
				...
				scope.pinCurrentSheet = function () {
					if (indexOfPinnedSheet(scope.sheet) !== -1) {
						return;
					}
				...
				scope.doAnnotationSync = function doAnnotationSync() {
					...
					scope.syncAnnotations(version, mod, del).then(function (result) {
						...
						scope.annotationWatcher = $interval(function () {
							scope.doAnnotationSync();
						}, BluVueSheet.Constants.ANNOTATION_SYNC_INTERVAL, 1);
					});
				};
					...
					for (var i in scope.attachmentFiles) {
						...
						switch (scope.attachmentFiles[i].annotation.type) {
							case SQUARE_ANNOTATION:
								scope.attachmentFiles[i].type_label = 'Square';
								break;
							case X_ANNOTATION:
								scope.attachmentFiles[i].type_label = 'X';
								break;
							case CIRCLE_ANNOTATION:
								scope.attachmentFiles[i].type_label = 'Circle';
								break;
							case ARROW_ANNOTATION:
								scope.attachmentFiles[i].type_label = 'Arrow';
								break;
							case CLOUD_ANNOTATION:
								scope.attachmentFiles[i].type_label = 'Cloud';
								break;
							case TEXT_ANNOTATION:
								scope.attachmentFiles[i].type_label = 'Text';
								break;
							case LINE_ANNOTATION:
								scope.attachmentFiles[i].type_label = 'Line';
								break;
							case PEN_ANNOTATION:
								scope.attachmentFiles[i].type_label = 'Pen';
								break;
							case HIGHLIGHTER_ANNOTATION:
								scope.attachmentFiles[i].type_label = 'Pencil';
								break;
							case SCALE_ANNOTATION:
								scope.attachmentFiles[i].type_label = 'Calibration';
								break;
							case MEASURE_ANNOTATION:
								scope.attachmentFiles[i].type_label = 'Distance';
								break;
							case POLYGON_ANNOTATION:
								scope.attachmentFiles[i].type_label = 'Polygon';
								break;
							case FREE_FORM_ANNOTATION:
								scope.attachmentFiles[i].type_label = 'Free-form';
								break;
						}
